...
Realizar un programa que maneje un archivo de longitud desconocida, donde cada registro contiene el nombre de un alumno y cuatro notas.
...
- modificar una nota,
- agregar registros,
- ordenarlo en el archivo alfabéticamente.

Realizar también el programa que genere el archivo y verificar que funcionen en conjunto.
...
**A.** Si el archivo es realmente un PNG válido, verificando los primeros 8 bytes:
...
- A continuación viene un chunk llamado `IHDR`. Su estructura comienza así:
  ...
  - Los `uint32_t` están en big-endian, también llamado network byte order. Como las PCs son little-endian, hay que invertir el orden de los bytes al leer.
